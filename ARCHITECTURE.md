...
`prefers-color-scheme`-scoped `theme-color` metas matching `--panel`. A single fixed
value left them clashing with the bar in one scheme. Those metas override the
manifest's `theme_color` and, unlike it, apply on reload rather than needing a
reinstall — which manifest changes do (Edge captures the manifest at install time).
