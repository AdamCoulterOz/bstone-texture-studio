...
for the close/minimise/zoom buttons. `interop.js` queries that rectangle rather
than assuming an inset — the buttons are on the left on macOS and the right on
Windows — and publishes it as `--titlebar-h/-l/-r` for the topbar to match
exactly, re-reading it on `geometrychange`.

The `.wco` class on `<html>` is the switch, and it is set from
`windowControlsOverlay.visible`, not from the API merely existing: a browser that
supports it still reports `visible: false` in an ordinary tab, where the topbar
has to keep its normal flow height. Chromium also refuses to honour `app-region`
outside an installed app window, so the drag behaviour cannot be exercised in a
tab even with the geometry faked.
