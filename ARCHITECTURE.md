...
The studio is a PWA: `manifest.webmanifest` plus a service worker that caches the
whole app, so it installs to the dock and runs offline. Everything is already
static and client-side, so this costs nothing architecturally.

Two workers, same filename:

- `service-worker.js` — development. Never caches (net11 fingerprints framework
  assets; a cached worker would serve the previous build after every rebuild).
- `service-worker.published.js` — swapped in at publish. Caches every asset in
  `service-worker-assets.js`, which the build regenerates with a fresh version
  hash each time. **That hash changing is the entire update-detection
  mechanism.**

Neither calls `skipWaiting()` on install, on purpose. A new worker parks in
"waiting" until the user accepts it, because taking over mid-session would
reload the page — and the studio holds unsaved in-flight work (a placement
session, a queued generation). `AppUpdateService` surfaces the offer as a yellow
**Update Available: Install** button in the title bar; accepting posts
`SKIP_WAITING` and reloads once the new worker takes control, and dismissing
keeps the current version until the *next* build.

The install step is the load-bearing part, and it fails silently. It caches every
asset with the integrity hash recorded at publish time, and `cache.addAll` is
all-or-nothing — so one deployed file whose bytes no longer match its recorded
hash makes install throw, and a worker that never installs never waits, so no
update is ever offered and offline never works. Nothing surfaces: the page runs
from the network as normal. Deploying to a project Pages site does exactly this,
because `index.html`'s `<base href>` is rewritten after publish. `tools/deploy.sh`
therefore re-stamps the manifest and refuses to push a tree whose hashes do not
match; `interop.js` logs a worker that goes `redundant`, which is what a failed
install looks like from the page.

## Window controls overlay

Installed, the app asks for `display_override: ["window-controls-overlay", …]`,
which hands the whole window to the page and leaves the OS only a small rectangle
for the window buttons.

There is **one** rectangle to query, the *usable* strip — not one per control
group — so each inset comes from an edge of it: the left is where the strip
starts, and the right is whatever is left between the strip's right edge and the
window. That matters on macOS in Edge, which has controls on *both* sides (traffic
lights on the left, its own `…` menu on the right).

Geometry comes from `env(titlebar-area-*)` first: it is the purpose-built channel,
populated in an installed window and by Edge DevTools' overlay simulator, and it
updates on resize without JS. `interop.js` publishes the same rectangle from
`getTitlebarAreaRect()` as `--titlebar-h/-l` for a fallback, and sets `.wco`. The
right inset is derived in CSS only — computing it in JS against `innerWidth` mixed
coordinate spaces with the rect and came out wrong.

The `.wco` class on `<html>` gates only the *structural* changes (nowrap, drag,
zeroed vertical padding), and is set from `windowControlsOverlay.visible`, not from
the API merely existing: a browser that supports it still reports `visible: false`
in an ordinary tab. Chromium refuses to honour `app-region` outside an installed
app window, so dragging cannot be exercised in a tab even with faked geometry.

The browser paints the reserved strips in the theme colour, so `index.html` carries
`prefers-color-scheme`-scoped `theme-color` metas. Those override the manifest's
`theme_color` and, unlike it, apply on reload rather than needing a reinstall —
which manifest changes do (Edge captures the manifest at install time).

**The metas must match `.wco .topbar`'s background, which is `--panel2`, not
`--panel`.** In overlay mode the bar is deliberately a shade off the content and
carries *no* hairline divider. Two attempts to place one — `border-bottom`, then a
`box-shadow` a pixel below the box — were both eaten at the ends, and neither could
be reproduced locally because a preview tab has no real control strips. A tone
change reads across the whole width, including under the strips, and does not
depend on any single row of pixels surviving.

Overlays that would otherwise cover the bar (`.drawer`, `.drawer-backdrop`) are
inset from the top by the same height, and the bar is raised to `z-index: 45` —
above the drawer's 41 — so a panel slides *under* it rather than spilling its
shadow across it, and stays undimmed by the backdrop. The bar is the window's only
drag handle once the OS title bar is gone, so a full-height panel would both make
the window immovable and hide its controls. Raising it makes `.topbar` a stacking
context, which confines the Settings menu (`60`) and its click-catching backdrop
(`59`); both still land above the drawer, since the whole context sits at 45.

The icon is generated by `tools/make-icons.py` rather than checked in as opaque
binaries — one geometry (a 2x2 checker), rendered directly at each size rather
than downsampled from 512. Re-run it after changing the design.
