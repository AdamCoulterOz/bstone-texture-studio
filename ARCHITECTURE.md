...
inset from the top by the same height. The bar is the window's only drag handle
once the OS title bar is gone, so a full-height panel makes the window immovable.
